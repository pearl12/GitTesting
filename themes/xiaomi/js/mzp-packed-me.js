    ...
    var loadingText='Loading Zoom';var loadingImg='';var iels=window.document.getElementsByTagName("IMG");for(var i=0;i<iels.length;i++){
        ...
        if(/MagicZoom/.test(aels[i].className)){
            while(aels[i].firstChild){
                ...
                }re=new RegExp(/zoom\-width(\s+)?:(\s+)?(\w+)/i);var zoomWidth=-1;matches=re.exec(aels[i].rel);bigCont.style.width='548px';if(matches){
                bigCont.style.width=matches[3];zoomWidth=matches[3]
                }re=new RegExp(/zoom\-height(\s+)?:(\s+)?(\w+)/i);var zoomHeight=-1;matches=re.exec(aels[i].rel);bigCont.style.height='440px';if(matches){
